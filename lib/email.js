...
async function sendOrderConfirmation(order) {
  ...
  const html = wrap(site, `Order #${order.number} confirmed`, `
...
async function sendOwnerOrderAlert(order) {
  if (!ADMIN_EMAIL) { console.log(`[email:no-admin-email] New order #${order.number} — set ADMIN_EMAIL to receive alerts.`); return; }
  const site = store.getSite();
  const html = wrap(site, `New order #${order.number}`, `
...
async function sendContactAlert(message) {
  if (!ADMIN_EMAIL) { console.log(`[email:no-admin-email] New contact message from ${message.email} — set ADMIN_EMAIL to receive alerts.`); return; }
  const site = store.getSite();
  const html = wrap(site, "New contact message", `
